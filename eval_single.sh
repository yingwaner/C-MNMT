...
declare -a testset=('de' 'fr' 'it' 'nl' 'ro')
declare -a testset1=('De' 'Fr' 'It' 'Nl' 'Ro')
declare -a refset=(
...
ALPHA=0.6
BATCH=250
DATABIN=/data/wanying/1.research/multilingual/data-bin/iwslt17
...
#    echo -e "\t$v\t$bleu"
    ...
    for i in $(seq 0 $max); do
        ...
        m=$CKPT_DIR/single_${T}baseline/checkpoint$1.pt
        m_name=`basename ${m}`
        echo $m_name
        cat /data/wanying/2.data/multilingual/DeEnItNlRo-DeEnItNlRo/preprocessed/fastBPE/iwslt17/tst.32k.$t-en.$t \
        | CUDA_VISIBLE_DEVICES=$GPU fairseq-interactive $DATABIN/$T-En --path $m \
        --task translation --gen-subset test \
        --buffer-size 2000 \
